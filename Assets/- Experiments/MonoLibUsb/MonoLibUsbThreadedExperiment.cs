...
// Resources:
// - MonoLibUsb reference http://libusbdotnet.sourceforge.net/V2/Index.html
// - SharpUsbLib http://www.icsharpcode.net/opensource/sharpusblib/
// - Multithreading under Unity http://answers.unity3d.com/questions/200176/multithreading-and-messaging.html

public enum CoordinateSystem {
	World, Camera, Self,
}
public class MonoLibUsbThreadedExperiment : MonoBehaviour {
		...
		if (Target) {
			switch (CoordinateSystem) {
				...
				case CoordinateSystem.Camera:
					Target.Translate(Translation, Camera.main.transform);

					// ToDo: This method has a different sensitivity than using Rotate()
					Target.RotateAround(Vector3.up, Rotation.y);
					Target.RotateAround(Camera.main.transform.right, Rotation.x);
					break;
	...
	private void SpaceNavigatorThread() {
		...
		while (!_quit) {
			ReadSpaceNavigator(ref Translation, ref Rotation, ref ReadBuffer);
